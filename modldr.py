...
__usage__ = """{}: [-l loader] [-p port] [-v] [-r response] inputfile
startrom:
...
arglist = 'l:p:vr:'
ppattern = re.compile(rb'\x6a\x04\x68(..)\x00\x00\xff\x35', re.DOTALL)
...
if __name__ == '__main__':
    ...
    if filetype != b'MZ':
        ...
        if not check_name(loader):
            ...
            print(allowed_chars)
        pat = re.compile(rb'NTLDR', re.IGNORECASE)
        out = pat.sub(loader.encode(), data)
        if out == data:
        ...
        if response is not None:
            ...
            if not check_name(response):
                ...
                print(allowed_chars)
            pat = re.compile(rb'winnt\.sif', re.IGNORECASE)
            out = pat.sub(response.encode(), data)
            if out == data:
